...
public final class Utilities {
    ...
    /// Initializes all the available
    // [`@Subcommand`][dev.floofy.noel.pinecone.annotations.Subcommand]s available
    ...
    public static HashMap<String, Subcommand> collectSubcommands(
            Class<?> clazz,
            Function3<
                            dev.floofy.noel.pinecone.annotations.Subcommand,
                            Method,
                            List<Option>,
                            Subcommand>
                    initializer) {
        ...
        for (Method method : clazz.getDeclaredMethods()) {
            final var annot =
                    method.getAnnotation(dev.floofy.noel.pinecone.annotations.Subcommand.class);
            if (annot == null) {
                continue;
            }

            log.trace(
                    "Found method {} with @Subcommand annotation in class `{}'",
                    method.getName(),
                    clazz);
            final var fields = method.getParameters();
            if (fields.length == 0) {
                log.warn(
                        "Method {} is not a viable subcommand: has no parameters",
                        method.getName());
                continue;
            }
            ...
            if (!CommandContext.class.isAssignableFrom(fields[0].getType())) {
                log.warn(
                        "Method {} is not a viable subcommand: first argument was not of class {}"
                            + " but {} instead",
                        method.getName(),
            ...
            for (int i = 1; i < fields.length; i++) {
                ...
                if (!param.isAnnotationPresent(dev.floofy.noel.pinecone.annotations.Option.class)) {
                    throw new IllegalArgumentException(
                            "Expected `@Option()` annotation to be present");
                }

                final dev.floofy.noel.pinecone.annotations.Option info =
                        param.getAnnotation(dev.floofy.noel.pinecone.annotations.Option.class);
                ...
                options.add(
                        new Option(
                                info, resolvedType, new ParameterTarget(i, param), isOptionalType));
            }

            final var name = annot.name().isEmpty() ? method.getName() : annot.name();
