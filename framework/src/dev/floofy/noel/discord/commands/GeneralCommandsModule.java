...
@Module(
        ...
        description = "Adds general commands like `/help` and `/ping`",
        priority = 100
)
public class GeneralCommandsModule extends AbstractNoelModule {
    @Override
    protected void configure() {
        final Multibinder<AbstractCommand> multibinder = Multibinder.newSetBinder(binder(), AbstractCommand.class);

        multibinder.addBinding().to(HelpCommand.class);
        multibinder.addBinding().to(PingCommand.class);
    }
}
